...
async def test_dedupe_subscription_skips_reentrant_events() -> None:
    ...
    scheduler.register_feature(
        FeatureSpec(
            name="dedupe_feature",
            subscriptions=[EventSubscription("evt", delivery_mode=DeliveryMode.DEDUPE, source=EventSource.INTERNAL)],
            handler=handler,
        )
    )
...
async def test_queue_subscription_buffers_reentrant_events() -> None:
    ...
    scheduler.register_feature(
        FeatureSpec(
            name="queue_feature",
            subscriptions=[EventSubscription("evt", delivery_mode=DeliveryMode.QUEUE, source=EventSource.INTERNAL)],
            handler=handler,
        )
    )
    await scheduler.start()
...
async def test_feature_can_publish_event_for_other_subscribers() -> None:
    ...
    scheduler.register_feature(
        FeatureSpec(
            name="source_feature",
            subscriptions=[EventSubscription.internal("evt.source", delivery_mode=DeliveryMode.QUEUE)],
    ...
    scheduler.register_feature(
        FeatureSpec(
            name="target_feature",
            subscriptions=[EventSubscription.internal("evt.target", delivery_mode=DeliveryMode.QUEUE)],
