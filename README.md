...
- `homeassistant/component/`：Rose Home Assistant 自定义集成源码
- `custom_components/rose/`：供 HACS 校验和安装的同步镜像目录
...
HACS 可以安装默认分支，也可以使用 GitHub Release 中固定名称的 `rose-home-assistant.zip`。维护者推送与 manifest 版本一致的 tag（例如 `v0.1.0`）后，GitHub Actions 自动创建 Release 和安装包。CI 会校验 `homeassistant/component` 与 `custom_components/rose` 完全一致。
