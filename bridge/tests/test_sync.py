...
from ..src import IoTAgentClient, EVENT_SYNC_RESPONSE, EVENT_PORT_STATUS
...
class TestSyncProtocol:
    ...
    def test_sync_followed_by_port_status_events(self, client):
        ...
        gpio5_events = [
            ...
            if hasattr(e, 'resource_type') and e.resource_type == 0 and hasattr(e, 'id') and e.id == 5
        ]
        assert len(gpio5_events) >= 1, (f"GPIO 5 not in port status snapshot after sync")
