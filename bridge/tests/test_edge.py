...
class TestGpioEdgeEvent:
    ...
    def test_gpio_interrupt_mode_generates_edge_events(self, client):
        ...
        assert len(rx_edges) > 0, (f"No edge events captured on GPIO {self.RX_PIN}. "
    ...
    def test_gpio_interrupt_edge_count_matches_toggles(self, client):
        ...
        assert len(edges) >= TOGGLES, (f"Expected >= {TOGGLES} edges on GPIO {self.RX_PIN}, got {len(edges)}")
