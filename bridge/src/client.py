...
from .protocol import EVENT_SYNC_RESPONSE, EVENT_GPIO_VALUE, EVENT_ADC_VALUE, EVENT_GPIO_SIGNAL_CAPTURED, EVENT_UART_RX, EVENT_HEARTBEAT, EVENT_BLE_PEERS_LIST, EVENT_BLE_PAIRING_ENABLED, RESOURCE_GPIO, RESOURCE_UART
...
class IoTAgentClient:
    ...
    def configure_uart(self, uart_id: int, baudrate: int, tx_gpio: int = 1, rx_gpio: int = 3,
        ...
        if retry_cmd_id is None:
            return None
        retry_response = self.events.wait_for_response(retry_cmd_id)
        if isinstance(retry_response, EventCmdAck) and retry_response.status == 0:
            listener = UartRxListener()
            self._uart_listeners[uart_id] = listener
            return listener
        return None
