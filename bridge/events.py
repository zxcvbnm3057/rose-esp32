...
class EventHandler:
    ...
    def __init__(self) -> None:
        self.event_callbacks: Dict[int, Callable[[Any], None]] = {}
    ...
    def handle_event(self, frame: MessageFrame) -> None:
        ...
        try:
            ...
            if event:
                ...
                # Correlate response by frame cmd_id for command-response events.
                with self._lock:
                    self.pending_responses.setdefault(frame.cmd_id, []).append(event)
                    self.pending_events.setdefault(opcode, []).append(event)

                # Also keep explicit command-id responses when provided in payload.
                if hasattr(event, 'cmd_id'):
                    with self._lock:
                        self.pending_responses.setdefault(event.cmd_id, []).append(event)

                # Call registered callback
                if opcode in self.event_callbacks:
                    self.event_callbacks[opcode](event)
                else:
                    logger.debug(f"Unhandled event type: {opcode}")
